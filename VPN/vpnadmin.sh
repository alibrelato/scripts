...
revoga_usuario() {
    ...
    if [ $(id -u) -eq 0 ]; then
		...
		echo "  CASO TENHA DUVIDA QUANTO AO NOME DO USUARIO VOLTE AO MENU INICIAL E CONSUTE A OPEÇÃO 6 "
		echo
...
altera_senha() {
	...
	echo
    echo "  FUNCAO DESATIVADA "
	echo "  ESSA FUNÇÃO SO SERA UTILIZADA APOS REEDUCAMOS OS USUARIOS ATUALMETE ELES ESTAO ACOSTUMADOS NO FORMATO ATUAL "
...
then
    procA=`ps aux | grep vpnadmin.sh | grep -v grep | awk ' { print $2 } '`
    kill -9 ${procA}
else
    opcao_invalida
    read
