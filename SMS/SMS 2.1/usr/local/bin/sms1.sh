...
maxErrorCount=2   ## Maximo de erros para o modem e SIM desativado - envio de alerta via email. default 24 (+/- 3 minutos)
			...
			if [ ${contagemErro} -ge ${maxErrorCountSim} ]; then
				...
				while true; do
					...
					if [ ! ${procA} ]; then
						...
						cat /usr/local/bin/redundanciaModem2.1_1.sh > /usr/local/bin/disparaSMS2.1.sh
