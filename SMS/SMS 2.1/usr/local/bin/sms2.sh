...
intervalNextTry=1 ## Tempo seg para aguardar cada tentativa/sucesso. default [5]
...
while [ ${Loop} == 1 ] && [ ! ${contagemErro} == ${maxErrorCount} ]; do
	...
	if [ ! -z ${voucherSQL} ] && [ ! -z ${telefoneSQL} ]; then
		...
		if [[ $filtroExecucao = *"OK"* ]]; then
			...
			query "UPDATE dados SET dia = '${diaEnvioSQL}', hora = '${horaEnvioSQL}', status = '${status}', envios = '${enviosSQL}'  WHERE voucher = '${voucherSQL}' AND telefone = '${telefoneSQL}';"
			##query "UPDATE dados SET hora='${horaEnvioSQL}' WHERE voucher='${voucherSQL}' AND telefone='${telefoneSQL}';"
		...
		## Se a variavel filtroExecucao nao tem a mensagem OK, entra no tratamento do erro ##
			...
			if [ ${contagemErro} -ge ${maxErrorCountSim} ]; then
				...
				while true; do
					...
					procA=$(ps axu | grep sms1 | grep -v grep)
					## Se o proce sms1 NAO existe, executa ##
					if [ ! ${procA} ]; then
						## Para o disparaSMS2 ##
						/etc/init.d/SMSD stop
						## Escreve o arquivo de redundancia no disparaSMS2 para nao executar mais ese script ##
						cat /usr/local/bin/redundanciaModem2.1_2.sh > /usr/local/bin/disparaSMS2.1.sh
						...
						printf "${horaEnvioSQL} ${filtroExecucao} \n" >> ${logFile}
						## Manda o erro por SMS ##
						echo "halia.alergs.br -- O modem 2 esta com problemas" |gammu -c /etc/gammu1.conf --sendsms TEXT 5199091414
						#
