		...
		## Se a variavel filtroExecucao nao tem a mensagem OK, entra no tratamento do erro ##
			...
			if [ ${contagemErro} -ge ${maxErrorCountModemSim} ]; then
				...
				while true; do
					...
					procA=`ps aux | grep sms1.sh | grep -v grep | wc -l`
					...
					if [ ${procA} -eq 0 ]; then
						...
						printf "${horaEnvio} ${filtroExecucao} mais de 4 erros para modem 2 \n" >> ${logFile}
						printf "================================================ \n" >> ${logFile}
