...
dataBase="smsGatewayTeste_Halia"
	...
	if [ ${voucherSQL} ]; then
		...
		if [[ "${telefoneSqlHoje}" = "${telefoneSQL}" ]];then
			...
			## Se nao a quantidade de voucher enviado eh maior que 3... ##
				...
				query "UPDATE dados SET status = '', controle = '9' WHERE voucher = '${voucherSqlHoje}' AND telefone = '${telefoneSqlHoje}';"
...
#teste2=$(ps axu | grep sms2.sh | grep -v grep | wc -l)
	...
	if [ ${teste1} -eq 0 ]; then
		...
		if [ ! -z ${voucherSqlModemA} ] && [ ! -z ${telefoneSqlModemA} ]; then
			...
			exec /usr/local/bin/teste/sms1.sh > /dev/null 2>&1 &
		fi
...
#			exec /usr/local/bin/teste/sms2.sh > /dev/null 2>&1 &
#		fi
	else
		sleep 0
	fi
